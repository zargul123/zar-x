...
**NEXT BUILD SESSION IS ALL ATTACK, NO BUILD: R-014 — a FIFTEENTH sabotage
against funding, a FOURTEENTH against Fear & Greed, a TENTH against the
open-interest recorder, invented by someone who built none of them.**
**CONTEXT: FIVE independent reviews in a row have each failed the repair before
them.** The newest, 2026-07-28 evening, found that three constants deciding what
each gate EXPECTED were read out of the module under test, so corrupting one
moved the lie and the bar together — and that `--record`, the branch the monthly
task actually runs, was executed by no test at all. **Four of four attacks
escaped. R-013 FAILED; R-014 is open against its repair; R-001 has now outlived
four generations of fix and moves only when one SURVIVES an independent attack.**
...
| **Open-interest recorder (Phase 3, Step 3.2b) — Binance 30-day window, `period=4h`, append-only CSV per asset, idempotent, never rewrites history. GATE 3.2b-R2 PASSED 2026-07-28 (evening): ten bars and NINE sabotages caught; the drill judges THE CSV ON DISK against a raw fetch for every asset, and the asset list is **the gate's own (`GATE_SYMBOLS`), not the module's**. Two independent sabotages walked through the previous gate the same day: **B9** cut `SYMBOLS` to two assets and SOLUSDT vanished from the recorder *and from its own detector*, permanently, all green; **B8** made `--record` — the branch the monthly task actually runs, which no test had ever executed — always exit 0, so a failed job reported success. New check (j) now runs `--record` for real as a subprocess in BOTH outcomes, against a copy in scratch. 540 rows recorded. SCHEDULED 2026-07-27: task `ZarX Open Interest`, day 1 monthly, 09:00, on the laptop only (Binance geo-blocks US cloud runners), catches up if the laptop was off. **R-013 FAILED. R-014 open against the repair.** Still known-weak: check (e) is BTCUSDT-only, B1 is a no-op on a UTC machine. | data/open_interest.py + data/oi_history/ | ✅ |
| Context Deck — instrument 1 of 5: Fear & Greed (alternative.me, free, keyless; injectable URL, fails to one offline line). **GATE 3.1-R4 (hardened 2026-07-28 evening): the self-test rebuilds the WHOLE printed block and requires EXACT equality on BOTH paths the pilot can see — live AND offline — and every constant it judges by is now typed out in the gate and compared to the module's by name: the disclaimer, the history limit, and (new) the offline wording. It breaks itself THIRTEEN ways every run, all thirteen caught. The offline bar was itself built from the MODULE'S `OFFLINE_WORDS` until 2026-07-28 evening, when F13 reworded that one constant so the lie and the bar moved together — the pilot's offline line read 'last known reading 72 — Extreme Greed' on a day the index read 29 — Fear, and the gate ticked it. **R-013 FAILED. R-014 open against the repair.** | cockpit/fear_greed.py | ✅ |
| Context Deck — instrument 2 of 5: funding rates (Binance USDⓈ-M public, free, keyless; USDT perpetuals, partial failure names the missing asset). **GATE 3.2-R4 (hardened 2026-07-28 evening): the self-test rebuilds the WHOLE printed block from Binance raw using its own arithmetic and requires EXACT equality on EVERY path the pilot can see — healthy, degraded and offline — holds its OWN copy of the "positive = longs pay shorts" wording, of the tickers, and (new) of the offline wording, each compared to the module's by name; rotates the partial-failure drill through all three assets; and breaks itself FOURTEEN ways every run, all fourteen caught. The offline bar was itself built from the MODULE'S `OFFLINE_WORDS` until 2026-07-28 evening, when S14 reworded that one constant and a fabricated "last reading BTC +0.0100%, longs paying" walked through with a tick mark reading "NOTHING appended". **R-013 FAILED. R-014 open against the repair.** Still unguarded: the block printed when TWO of three assets fail is built by no check anywhere. | cockpit/funding.py | ✅ |
