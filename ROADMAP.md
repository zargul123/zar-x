...
**NEXT BUILD SESSION IS ALL ATTACK, NO BUILD: R-015 — a SIXTEENTH sabotage
against funding, a FIFTEENTH against Fear & Greed, an ELEVENTH against the
open-interest recorder, invented by someone who built none of them.**
**CONTEXT: SIX independent reviews in a row have each failed the repair before
them.** The newest, 2026-07-28 night, found that every check on this ship
inspects a RETURN VALUE, or a file it just created FROM EMPTY. So a doorway that
simply `print()`s advice reached the Brief completely unwatched — `brief.py` runs
